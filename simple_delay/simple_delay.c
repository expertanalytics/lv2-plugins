...
#include <stdlib.h>
#include <stdio.h>
#include "amplification_lookup.h"
...
#define DB_CO_old(g) ((g) > -90.0f ? powf(10.0f, (g)*0.05f) : 0.0f)
...
run(LV2_Handle instance, uint32_t n_samples)
{
    ...
    //const float output_gain = DB_CO_old(*(delay->output_gain));
    const float output_gain = lookup(*(delay->output_gain), DB_C0);
    int delay_pos_input;
    float delay_signal;
    float y1;
    float y2;
